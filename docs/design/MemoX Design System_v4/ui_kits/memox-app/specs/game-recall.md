# game-recall — Recall — DOM spec

Auto-generated by `tool/ui_kit_shots/export_specs.mjs` from the rendered UI kit. Do not
edit by hand; re-run the exporter after any `../index.html` change (the freshness check
in `tool/verify/run.mjs` fails when this is stale).

Reading guide: each visible node is a block; one short head line, then grouped field lines.
`- [item[i]] name "own text" mx:<Mx> abs:[x,y WxH] rel:[x,y WxH] <layout> <flex-child> repeat:xN(unit=P) pad:t/r/b/l margin:t/r/b/l minw/maxw/minh/maxh pos:... layout_hint:... z:N scrollh:N transform:... bg:<color> font:<size/weight[/line-height]> color:<color> text:<align> tracking:N r:<radius> border:<w>px <color> (or border-t/r/b/l for a single-side divider) shadow:<offY>/<blur>`.
Indentation = DOM containment (layout/grouping containers are kept, not flattened).
`abs:[...]` is frame-relative (cross-check with the PNG); `rel:[...]` is the box offset+size
INSIDE its parent - read spacing from rel, not abs, so the layout stays relative.
`<layout>` on a container is its child arrangement: `flex:row|col gap:N justify:... align:...`
or `grid cols:N` - map to a Flutter Row/Column/Wrap/GridView, not absolute coords.
`<flex-child>` is a flex item constraint: `grow:N shrink:N basis:N self:...` plus
`layout_hint:expanded` (-> Expanded) / `layout_hint:flexible` (-> Flexible).
`pad`/`margin` are 4-edge (collapsed: `N` all-equal, `V/H`, or `t/r/b/l`); `minw/maxw/minh/maxh`
are explicit size constraints. `pos:` is non-static positioning; `layout_hint:scroll` =
scroll container, `layout_hint:pinned` = sticky/fixed (bottom bars, sheets, FABs), `clip` =
overflow hidden, `z:N` = stacking - use these to decide Stack/Positioned/bottomSheet vs flow.
`repeat:xN(unit=P)` marks a list of N items of P elements each; `item[i]` tags each unit
start - build it as a list/builder, not N copies (a +N suffix means a trailing partial unit).
`scrollh:N` is the scroll content height (vs the viewport `WxH`); `transform:...`,
`tracking:N` (letter-spacing px), `text:<align>` are emitted only when set.
`shadow:<offY>/<blur>` is the box-shadow -> map to an elevation. Coordinates are px on the
390x780 phone frame (light theme measured; dark remaps the same `--memox-*` tokens). A
`<color>` is a `--memox-*` token name; `token@NN` / `#rrggbb@NN` = that color at NN% opacity
(overlay/tint, not a hardcoded color). Token names map to the Flutter theme tokens; a bare
`#rrggbb` means no token matched - treat as a gap, not a license to hardcode. Non-base states are an ordered diff (`+` added / `-` removed
in document order with abs+rel bbox kept, `...` = unchanged run). Every quoted "..." string is
MOCK COPY - the kit carries NO l10n keys; never copy it into the app, source real strings from
the app ARB (`lib/l10n/`). Numbers/counts are illustrative, not the system contract. `mx:<Mx>`
is the suggested MemoX shared component (the per-screen identity contract is
`tool/parity/contracts/`); `mx:?` is an interactive control with no confident mapping. When no `mx:` is present, `name` is just the
raw kit CSS class (e.g. `ov`, `title`) and is NOT a resolved component. Two mappings stay
deliberately MISSING, not guessed: a bare `#rrggbb` is an un-tokenized color, and quoted text
has no l10n key. Visual reference PNGs: `../shots/` (see `../shots/INDEX.md`).

## Base state: before reveal

```text
- node: app
  id: game-recall/screen
  box:
    abs: [0,0 390x780]
    rel: [0,0 390x780]
  layout: flex:col
  position: pos:relative clip
  style: bg:bg
  - node: appbar
    id: game-recall/appbar
    box:
      abs: [0,0 390x64]
      rel: [0,0 390x64]
    layout: flex:row gap:12 align:center
    flex: shrink:0
    spacing: pad:0/20
    style: bg:bg
    - node: appbar__lead
      box:
        abs: [20,8 48x48]
        rel: [20,8 48x48]
      layout: flex:row gap:4 align:center
      - node: icon-button:arrow_back
        id: game-recall/back
        mx: ?
        box:
          abs: [20,8 48x48]
          rel: [0,0 48x48]
        layout: flex:row justify:center align:center
        spacing: pad:1/6
        style: r:9999
        - node: icon:arrow_back
          text: arrow_back
          box:
            abs: [32,20 24x24]
            rel: [12,12 24x24]
          style: font:24/400 color:text text:center
    - node: appbar__title
      text: Recall
      box:
        abs: [80,17 230x30]
        rel: [80,17 230x30]
      flex: grow:1 basis:0 layout_hint:expanded
      position: clip
      style: font:20/700/30 color:text tracking:-0.4
    - node: appbar__trail
      box:
        abs: [322,8 48x48]
        rel: [322,8 48x48]
      layout: flex:row gap:4 align:center
      - node: icon-button:more_horiz
        id: game-recall/options
        mx: ?
        box:
          abs: [322,8 48x48]
          rel: [0,0 48x48]
        layout: flex:row justify:center align:center
        spacing: pad:1/6
        style: r:9999
        - node: icon:more_horiz
          text: more_horiz
          box:
            abs: [334,20 24x24]
            rel: [12,12 24x24]
          style: font:24/400 color:text text:center
  - node: app__body
    box:
      abs: [0,64 390x716]
      rel: [0,64 390x716]
    layout: flex:col gap:20
    flex: grow:1 basis:0 layout_hint:expanded
    spacing: pad:16/20/96/20
    position: layout_hint:scroll
    - node: div
      id: game-recall/progress
      box:
        abs: [20,80 350x8]
        rel: [20,16 350x8]
      flex: shrink:0
      position: clip
      style: bg:surface-sunken r:999
      - node: div
        box:
          abs: [20,80 210x8]
          rel: [0,0 210x8]
        style: bg:primary r:999
    - node: card
      id: game-recall/term
      box:
        abs: [20,108 350x180]
        rel: [20,44 350x180]
      layout: flex:col gap:12 align:center
      flex: shrink:0
      spacing: pad:24
      position: pos:relative
      style: bg:surface r:20 shadow:9/16
      - node: div
        text: 친구
        box:
          abs: [152,132 86x72]
          rel: [132,24 86x72]
        style: font:48/800/72 color:text text:center tracking:-1
      - node: div
        box:
          abs: [149,216 92x48]
          rel: [129,108 92x48]
        layout: flex:row gap:8
        - node: icon-button:volume_up
          id: game-recall/audio
          mx: ?
          box:
            abs: [149,216 48x48]
            rel: [0,0 48x48]
          layout: flex:row justify:center align:center
          spacing: pad:1/6
          style: r:9999
          - node: icon:volume_up
            text: volume_up
            box:
              abs: [161,228 24x24]
              rel: [12,12 24x24]
            style: font:24/400 color:text text:center
        - node: icon-button:edit
          id: game-recall/edit
          mx: ?
          box:
            abs: [205,216 36x36]
            rel: [56,0 36x36]
          layout: flex:row justify:center align:center
          spacing: pad:1/6
          style: r:9999
          - node: icon:edit
            text: edit
            box:
              abs: [213,224 20x20]
              rel: [8,8 20x20]
            style: font:20/400 color:text text:center
    - node: card
      id: game-recall/meaning
      box:
        abs: [20,308 350x120]
        rel: [20,244 350x120]
      layout: flex:col gap:8 justify:center align:center
      flex: shrink:0
      spacing: pad:20
      size: minh:120
      position: pos:relative
      style: bg:surface r:20 shadow:9/16
      - node: div
        text: Recall the meaning, then tap “Show”
        box:
          abs: [68,358 253x20]
          rel: [48,50 253x20]
        layout: flex:row gap:8 align:center
        style: font:13/600/20 color:text-tertiary text:center
        - node: icon:visibility
          text: visibility
          box:
            abs: [68,359 18x18]
            rel: [0,1 18x18]
          style: font:18/400 color:text-tertiary text:center
    - node: btn
      id: game-recall/reveal
      mx: ?
      box:
        abs: [20,448 350x56]
        rel: [20,384 350x56]
      layout: flex:row gap:8 justify:center align:center
      flex: shrink:0
      spacing: pad:0/32
      size: minh:56
      style: bg:primary r:12 shadow:8/18
      - node: icon:visibility
        text: visibility
        box:
          abs: [157,466 20x20]
          rel: [137,18 20x20]
        style: font:20/400 color:surface text:center
      - node: span
        text: Show
        box:
          abs: [185,465 47x22]
          rel: [165,17 47x22]
        style: font:17/700 color:surface text:center
```

## State: revealed (ordered diff vs before reveal)

```diff
  - node: card
  id: game-recall/meaning
  box:
  abs: [20,308 350x128]
  rel: [20,244 350x128]
  layout: flex:col gap:8 justify:center align:center
  flex: shrink:0
  spacing: pad:20
  size: minh:120
  position: pos:relative
  style: bg:surface r:20 shadow:9/16
- - node: div
- text: Recall the meaning, then tap “Show”
- box:
- abs: [68,358 253x20]
- rel: [48,50 253x20]
- layout: flex:row gap:8 align:center
- style: font:13/600/20 color:text-tertiary text:center
- - node: icon:visibility
- text: visibility
- box:
- abs: [68,359 18x18]
- rel: [0,1 18x18]
- style: font:18/400 color:text-tertiary text:center
- - node: btn
- id: game-recall/reveal
- mx: ?
- box:
- abs: [20,448 350x56]
- rel: [20,384 350x56]
- layout: flex:row gap:8 justify:center align:center
- flex: shrink:0
- spacing: pad:0/32
- size: minh:56
- style: bg:primary r:12 shadow:8/18
- - node: icon:visibility
- text: visibility
- box:
- abs: [157,466 20x20]
- rel: [137,18 20x20]
- style: font:20/400 color:surface text:center
- - node: span
- text: Show
- box:
- abs: [185,465 47x22]
- rel: [165,17 47x22]
- style: font:17/700 color:surface text:center
+ - node: div
+ box:
+ abs: [167,328 56x4]
+ rel: [147,20 56x4]
+ style: bg:divider r:6
+ - node: div
+ text: friend
+ box:
+ abs: [151,340 87x45]
+ rel: [131,32 87x45]
+ style: font:30/700/45 color:text text:center
+ - node: div
+ text: a friend, companion
+ box:
+ abs: [124,393 141x23]
+ rel: [104,85 141x23]
+ style: font:15/400/23 color:text-secondary text:center
+ - node: div
+ box:
+ abs: [20,456 350x48]
+ rel: [20,392 350x48]
+ layout: grid cols:2 gap:12
+ flex: shrink:0
+ - node: btn
+ id: game-recall/forgot
+ mx: ?
+ box:
+ abs: [20,456 169x48]
+ rel: [0,0 169x48]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/24
+ size: minh:48
+ style: r:12
+ - node: span
+ text: Forgot
+ box:
+ abs: [80,470 50x19]
+ rel: [60,15 50x19]
+ style: font:15/700 color:primary-strong text:center
+ - node: btn
+ id: game-recall/remembered
+ mx: ?
+ box:
+ abs: [201,456 169x48]
+ rel: [181,0 169x48]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/24
+ size: minh:48
+ style: r:12 shadow:0/0
+ - node: span
+ text: Got it
+ box:
+ abs: [265,470 41x19]
+ rel: [64,15 41x19]
+ style: font:15/700 color:text text:center
```

## State: forgot (ordered diff vs before reveal)

```diff
  - node: card
  id: game-recall/meaning
  box:
  abs: [20,308 350x128]
  rel: [20,244 350x128]
  layout: flex:col gap:8 justify:center align:center
  flex: shrink:0
  spacing: pad:20
  size: minh:120
  position: pos:relative
  style: bg:surface r:20 shadow:9/16
- - node: div
- text: Recall the meaning, then tap “Show”
- box:
- abs: [68,358 253x20]
- rel: [48,50 253x20]
- layout: flex:row gap:8 align:center
- style: font:13/600/20 color:text-tertiary text:center
- - node: icon:visibility
- text: visibility
- box:
- abs: [68,359 18x18]
- rel: [0,1 18x18]
- style: font:18/400 color:text-tertiary text:center
- - node: btn
- id: game-recall/reveal
- mx: ?
- box:
- abs: [20,448 350x56]
- rel: [20,384 350x56]
- layout: flex:row gap:8 justify:center align:center
- flex: shrink:0
- spacing: pad:0/32
- size: minh:56
- style: bg:primary r:12 shadow:8/18
- - node: icon:visibility
- text: visibility
- box:
- abs: [157,466 20x20]
- rel: [137,18 20x20]
- style: font:20/400 color:surface text:center
- - node: span
- text: Show
- box:
- abs: [185,465 47x22]
- rel: [165,17 47x22]
- style: font:17/700 color:surface text:center
+ - node: div
+ box:
+ abs: [167,328 56x4]
+ rel: [147,20 56x4]
+ style: bg:divider r:6
+ - node: div
+ text: friend
+ box:
+ abs: [151,340 87x45]
+ rel: [131,32 87x45]
+ style: font:30/700/45 color:text text:center
+ - node: div
+ text: a friend, companion
+ box:
+ abs: [124,393 141x23]
+ rel: [104,85 141x23]
+ style: font:15/400/23 color:text-secondary text:center
+ - node: div
+ text: You'll see this word again this round.
+ box:
+ abs: [20,456 350x44]
+ rel: [20,392 350x44]
+ layout: flex:row gap:8 align:center
+ flex: shrink:0
+ spacing: pad:12/16
+ style: bg:warning-soft font:13/600/20 color:on-warning-soft r:12
+ - node: icon:replay
+ text: replay
+ box:
+ abs: [36,468 18x18]
+ rel: [16,13 18x18]
+ style: font:18/400 color:on-warning-soft
+ - node: div
+ box:
+ abs: [20,519 350x48]
+ rel: [20,455 350x48]
+ layout: grid cols:2 gap:12
+ flex: shrink:0
+ - node: btn
+ id: game-recall/forgot
+ mx: ?
+ box:
+ abs: [20,519 169x48]
+ rel: [0,0 169x48]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/24
+ size: minh:48
+ style: bg:error r:12
+ - node: span
+ text: Forgot
+ box:
+ abs: [80,534 50x19]
+ rel: [60,15 50x19]
+ style: font:15/700 color:surface text:center
+ - node: btn
+ id: game-recall/remembered
+ mx: ?
+ box:
+ abs: [201,519 169x48]
+ rel: [181,0 169x48]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/24
+ size: minh:48
+ style: r:12 shadow:0/0
+ - node: span
+ text: Got it
+ box:
+ abs: [265,534 41x19]
+ rel: [64,15 41x19]
+ style: font:15/700 color:text text:center
```

## State: remembered (ordered diff vs before reveal)

```diff
  - node: card
  id: game-recall/meaning
  box:
  abs: [20,308 350x128]
  rel: [20,244 350x128]
  layout: flex:col gap:8 justify:center align:center
  flex: shrink:0
  spacing: pad:20
  size: minh:120
  position: pos:relative
  style: bg:surface r:20 shadow:9/16
- - node: div
- text: Recall the meaning, then tap “Show”
- box:
- abs: [68,358 253x20]
- rel: [48,50 253x20]
- layout: flex:row gap:8 align:center
- style: font:13/600/20 color:text-tertiary text:center
- - node: icon:visibility
- text: visibility
- box:
- abs: [68,359 18x18]
- rel: [0,1 18x18]
- style: font:18/400 color:text-tertiary text:center
- - node: btn
- id: game-recall/reveal
- mx: ?
- box:
- abs: [20,448 350x56]
- rel: [20,384 350x56]
- layout: flex:row gap:8 justify:center align:center
- flex: shrink:0
- spacing: pad:0/32
- size: minh:56
- style: bg:primary r:12 shadow:8/18
- - node: icon:visibility
- text: visibility
- box:
- abs: [157,466 20x20]
- rel: [137,18 20x20]
- style: font:20/400 color:surface text:center
- - node: span
- text: Show
- box:
- abs: [185,465 47x22]
- rel: [165,17 47x22]
- style: font:17/700 color:surface text:center
+ - node: div
+ box:
+ abs: [167,328 56x4]
+ rel: [147,20 56x4]
+ style: bg:divider r:6
+ - node: div
+ text: friend
+ box:
+ abs: [151,340 87x45]
+ rel: [131,32 87x45]
+ style: font:30/700/45 color:text text:center
+ - node: div
+ text: a friend, companion
+ box:
+ abs: [124,393 141x23]
+ rel: [104,85 141x23]
+ style: font:15/400/23 color:text-secondary text:center
+ - node: div
+ text: Nice! Moving to the next card.
+ box:
+ abs: [20,456 350x44]
+ rel: [20,392 350x44]
+ layout: flex:row gap:8 align:center
+ flex: shrink:0
+ spacing: pad:12/16
+ style: bg:success-soft font:13/600/20 color:on-success-soft r:12
+ - node: icon:check_circle
+ text: check_circle
+ box:
+ abs: [36,468 18x18]
+ rel: [16,13 18x18]
+ style: font:18/400 color:on-success-soft
+ - node: div
+ box:
+ abs: [20,519 350x48]
+ rel: [20,455 350x48]
+ layout: grid cols:2 gap:12
+ flex: shrink:0
+ - node: btn
+ id: game-recall/forgot
+ mx: ?
+ box:
+ abs: [20,519 169x48]
+ rel: [0,0 169x48]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/24
+ size: minh:48
+ style: r:12
+ - node: span
+ text: Forgot
+ box:
+ abs: [80,534 50x19]
+ rel: [60,15 50x19]
+ style: font:15/700 color:primary-strong text:center
+ - node: btn
+ id: game-recall/remembered
+ mx: ?
+ box:
+ abs: [201,519 169x48]
+ rel: [181,0 169x48]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/24
+ size: minh:48
+ style: bg:primary r:12 shadow:8/18
+ - node: span
+ text: Got it
+ box:
+ abs: [265,534 41x19]
+ rel: [64,15 41x19]
+ style: font:15/700 color:surface text:center
```

## State: complete (ordered diff vs before reveal)

```diff
  - node: div
  box:
  abs: [20,80 350x8]
  rel: [0,0 350x8]
  style: bg:primary r:999
- - node: card
- id: game-recall/term
- box:
- abs: [20,108 350x180]
- rel: [20,44 350x180]
- layout: flex:col gap:12 align:center
- flex: shrink:0
- spacing: pad:24
- position: pos:relative
- style: bg:surface r:20 shadow:9/16
- - node: div
- text: 친구
- box:
- abs: [152,132 86x72]
- rel: [132,24 86x72]
- style: font:48/800/72 color:text text:center tracking:-1
- - node: div
- box:
- abs: [149,216 92x48]
- rel: [129,108 92x48]
- layout: flex:row gap:8
- - node: icon-button:volume_up
- id: game-recall/audio
- mx: ?
- box:
- abs: [149,216 48x48]
- rel: [0,0 48x48]
- layout: flex:row justify:center align:center
- spacing: pad:1/6
- style: r:9999
- - node: icon:volume_up
- text: volume_up
- box:
- abs: [161,228 24x24]
- rel: [12,12 24x24]
- style: font:24/400 color:text text:center
- - node: icon-button:edit
- id: game-recall/edit
- mx: ?
- box:
- abs: [205,216 36x36]
- rel: [56,0 36x36]
- layout: flex:row justify:center align:center
- spacing: pad:1/6
- style: r:9999
- - node: icon:edit
- text: edit
- box:
- abs: [213,224 20x20]
- rel: [8,8 20x20]
- style: font:20/400 color:text text:center
- - node: card
- id: game-recall/meaning
- box:
- abs: [20,308 350x120]
- rel: [20,244 350x120]
- layout: flex:col gap:8 justify:center align:center
- flex: shrink:0
- spacing: pad:20
- size: minh:120
- position: pos:relative
- style: bg:surface r:20 shadow:9/16
- - node: div
- text: Recall the meaning, then tap “Show”
- box:
- abs: [68,358 253x20]
- rel: [48,50 253x20]
- layout: flex:row gap:8 align:center
- style: font:13/600/20 color:text-tertiary text:center
- - node: icon:visibility
- text: visibility
- box:
- abs: [68,359 18x18]
- rel: [0,1 18x18]
- style: font:18/400 color:text-tertiary text:center
- - node: btn
- id: game-recall/reveal
- mx: ?
- box:
- abs: [20,448 350x56]
- rel: [20,384 350x56]
- layout: flex:row gap:8 justify:center align:center
- flex: shrink:0
- spacing: pad:0/32
- size: minh:56
- style: bg:primary r:12 shadow:8/18
- - node: icon:visibility
- text: visibility
- box:
- abs: [157,466 20x20]
- rel: [137,18 20x20]
- style: font:20/400 color:surface text:center
- - node: span
- text: Show
- box:
- abs: [185,465 47x22]
- rel: [165,17 47x22]
- style: font:17/700 color:surface text:center
+ - node: div
+ id: game-recall/complete
+ box:
+ abs: [20,108 350x576]
+ rel: [20,44 350x576]
+ layout: flex:col gap:16 justify:center align:center
+ flex: grow:1 basis:0 layout_hint:expanded
+ spacing: pad:40/16
+ - node: icon-tile
+ box:
+ abs: [165,285 60x60]
+ rel: [145,177 60x60]
+ layout: flex:row justify:center align:center
+ flex: shrink:0
+ style: bg:success-soft r:18
+ - node: icon:celebration
+ text: celebration
+ box:
+ abs: [179,299 32x32]
+ rel: [14,14 32x32]
+ style: font:32/400 color:on-success-soft text:center
+ - node: div
+ box:
+ abs: [85,361 220x83]
+ rel: [65,253 220x83]
+ layout: flex:col gap:8
+ size: maxw:220
+ - node: div
+ text: Round complete!
+ box:
+ abs: [85,361 220x30]
+ rel: [0,0 220x30]
+ style: font:20/800/30 color:text text:center tracking:-0.4
+ - node: div
+ text: You've reviewed the words in this round.
+ box:
+ abs: [85,399 220x45]
+ rel: [0,38 220x45]
+ style: font:15/400/23 color:text-secondary text:center
+ - node: btn
+ id: game-recall/next
+ mx: ?
+ box:
+ abs: [117,460 157x48]
+ rel: [97,352 157x48]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/24
+ size: minh:48
+ style: bg:primary r:12 shadow:8/18
+ - node: icon:arrow_forward
+ text: arrow_forward
+ box:
+ abs: [141,474 20x20]
+ rel: [24,14 20x20]
+ style: font:20/400 color:surface text:center
+ - node: span
+ text: Next round
+ box:
+ abs: [169,474 81x19]
+ rel: [52,15 81x19]
+ style: font:15/700 color:surface text:center
```
