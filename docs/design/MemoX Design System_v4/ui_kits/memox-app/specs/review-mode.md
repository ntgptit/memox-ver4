# review — Review — DOM spec

Auto-generated by `tool/ui_kit_shots/export_specs.mjs` from the rendered UI kit. Do not
edit by hand; re-run the exporter after any `../index.html` change (the freshness check
in `tool/verify/run.mjs` fails when this is stale).

Reading guide: each visible node is a block; one short head line, then grouped field lines.
`- [item[i]] name "own text" mx:<Mx> abs:[x,y WxH] rel:[x,y WxH] <layout> <flex-child> repeat:xN(unit=P) pad:t/r/b/l margin:t/r/b/l minw/maxw/minh/maxh pos:... layout_hint:... z:N scrollh:N transform:... bg:<color> font:<size/weight[/line-height]> color:<color> text:<align> tracking:N r:<radius> border:<w>px <color> (or border-t/r/b/l for a single-side divider) shadow:<offY>/<blur>`.
Indentation = DOM containment (layout/grouping containers are kept, not flattened).
`abs:[...]` is frame-relative (cross-check with the PNG); `rel:[...]` is the box offset+size
INSIDE its parent - read spacing from rel, not abs, so the layout stays relative.
`<layout>` on a container is its child arrangement: `flex:row|col gap:N justify:... align:...`
or `grid cols:N` - map to a Flutter Row/Column/Wrap/GridView, not absolute coords.
`<flex-child>` is a flex item constraint: `grow:N shrink:N basis:N self:...` plus
`layout_hint:expanded` (-> Expanded) / `layout_hint:flexible` (-> Flexible).
`pad`/`margin` are 4-edge (collapsed: `N` all-equal, `V/H`, or `t/r/b/l`); `minw/maxw/minh/maxh`
are explicit size constraints. `pos:` is non-static positioning; `layout_hint:scroll` =
scroll container, `layout_hint:pinned` = sticky/fixed (bottom bars, sheets, FABs), `clip` =
overflow hidden, `z:N` = stacking - use these to decide Stack/Positioned/bottomSheet vs flow.
`repeat:xN(unit=P)` marks a list of N items of P elements each; `item[i]` tags each unit
start - build it as a list/builder, not N copies (a +N suffix means a trailing partial unit).
`scrollh:N` is the scroll content height (vs the viewport `WxH`); `transform:...`,
`tracking:N` (letter-spacing px), `text:<align>` are emitted only when set.
`shadow:<offY>/<blur>` is the box-shadow -> map to an elevation. Coordinates are px on the
390x780 phone frame (light theme measured; dark remaps the same `--memox-*` tokens). A
`<color>` is a `--memox-*` token name; `token@NN` / `#rrggbb@NN` = that color at NN% opacity
(overlay/tint, not a hardcoded color). Token names map to the Flutter theme tokens; a bare
`#rrggbb` means no token matched - treat as a gap, not a license to hardcode. Non-base states are an ordered diff (`+` added / `-` removed
in document order with abs+rel bbox kept, `...` = unchanged run). Every quoted "..." string is
MOCK COPY - the kit carries NO l10n keys; never copy it into the app, source real strings from
the app ARB (`lib/l10n/`). Numbers/counts are illustrative, not the system contract. `mx:<Mx>`
is the suggested MemoX shared component (the per-screen identity contract is
`tool/parity/contracts/`); `mx:?` is an interactive control with no confident mapping. When no `mx:` is present, `name` is just the
raw kit CSS class (e.g. `ov`, `title`) and is NOT a resolved component. Two mappings stay
deliberately MISSING, not guessed: a bare `#rrggbb` is an un-tokenized color, and quoted text
has no l10n key. Visual reference PNGs: `../shots/` (see `../shots/INDEX.md`).

## Base state: browsing

```text
- node: app
  id: review-mode/screen
  box:
    abs: [0,0 390x780]
    rel: [0,0 390x780]
  layout: flex:col
  position: pos:relative clip
  style: bg:bg
  - node: appbar
    id: review-mode/appbar
    box:
      abs: [0,0 390x64]
      rel: [0,0 390x64]
    layout: flex:row gap:12 align:center
    flex: shrink:0
    spacing: pad:0/20
    style: bg:bg
    - node: appbar__lead
      box:
        abs: [20,8 48x48]
        rel: [20,8 48x48]
      layout: flex:row gap:4 align:center
      - node: icon-button:arrow_back
        id: review-mode/back
        mx: ?
        box:
          abs: [20,8 48x48]
          rel: [0,0 48x48]
        layout: flex:row justify:center align:center
        spacing: pad:1/6
        style: r:9999
        - node: icon:arrow_back
          text: arrow_back
          box:
            abs: [32,20 24x24]
            rel: [12,12 24x24]
          style: font:24/400 color:text text:center
    - node: appbar__title
      text: Review
      box:
        abs: [80,17 178x30]
        rel: [80,17 178x30]
      flex: grow:1 basis:0 layout_hint:expanded
      position: clip
      style: font:20/700/30 color:text tracking:-0.4
    - node: appbar__trail
      box:
        abs: [270,8 100x48]
        rel: [270,8 100x48]
      layout: flex:row gap:4 align:center
      - node: icon-button:format_size
        id: review-mode/text-size
        mx: ?
        box:
          abs: [270,8 48x48]
          rel: [0,0 48x48]
        layout: flex:row justify:center align:center
        spacing: pad:1/6
        style: r:9999
        - node: icon:format_size
          text: format_size
          box:
            abs: [282,20 24x24]
            rel: [12,12 24x24]
          style: font:24/400 color:text text:center
      - node: icon-button:more_vert
        id: review-mode/options
        mx: ?
        box:
          abs: [322,8 48x48]
          rel: [52,0 48x48]
        layout: flex:row justify:center align:center
        spacing: pad:1/6
        style: r:9999
        - node: icon:more_vert
          text: more_vert
          box:
            abs: [334,20 24x24]
            rel: [12,12 24x24]
          style: font:24/400 color:text text:center
  - node: app__body
    box:
      abs: [0,64 390x716]
      rel: [0,64 390x716]
    layout: flex:col gap:20
    flex: grow:1 basis:0 layout_hint:expanded
    spacing: pad:16/20/96/20
    position: layout_hint:scroll
    - node: div
      box:
        abs: [20,80 350x20]
        rel: [20,16 350x20]
      layout: flex:row gap:12 align:center
      flex: shrink:0
      - node: div
        id: review-mode/progress
        box:
          abs: [20,87 307x6]
          rel: [0,7 307x6]
        position: clip
        style: bg:surface-sunken r:999
        - node: div
          box:
            abs: [20,87 108x6]
            rel: [0,0 108x6]
          style: bg:primary r:999
      - node: span
        text: 7/20
        box:
          abs: [339,80 31x20]
          rel: [319,0 31x20]
        style: font:13/700/20 color:text-secondary
    - node: card
      id: review-mode/meaning
      box:
        abs: [20,120 350x133]
        rel: [20,56 350x133]
      layout: flex:col gap:12
      flex: shrink:0
      spacing: pad:20
      position: pos:relative
      style: bg:surface r:20 shadow:9/16
      - node: div
        box:
          abs: [40,140 310x36]
          rel: [20,20 310x36]
        layout: flex:row align:center
        - node: span
          text: MEANING
          box:
            abs: [40,148 274x20]
            rel: [0,8 274x20]
          flex: grow:1 basis:0 layout_hint:expanded
          style: font:13/700/20 color:text-tertiary tracking:0.5
        - node: icon-button:edit
          id: review-mode/edit
          mx: ?
          box:
            abs: [314,140 36x36]
            rel: [274,0 36x36]
          layout: flex:row justify:center align:center
          spacing: pad:1/6
          style: r:9999
          - node: icon:edit
            text: edit
            box:
              abs: [322,148 20x20]
              rel: [8,8 20x20]
            style: font:20/400 color:text text:center
      - node: div
        text: school
        box:
          abs: [40,188 310x45]
          rel: [20,68 310x45]
        style: font:30/700/45 color:text
    - node: card
      id: review-mode/term
      box:
        abs: [20,273 350x180]
        rel: [20,209 350x180]
      layout: flex:col gap:12 align:center
      flex: shrink:0
      spacing: pad:24
      position: pos:relative
      style: bg:surface r:20 shadow:9/16
      - node: div
        text: 학교
        box:
          abs: [152,297 86x72]
          rel: [132,24 86x72]
        style: font:48/800/72 color:text text:center tracking:-1
      - node: icon-button:volume_up
        id: review-mode/audio
        mx: ?
        box:
          abs: [171,381 48x48]
          rel: [151,108 48x48]
        layout: flex:row justify:center align:center
        spacing: pad:1/6
        style: r:9999
        - node: icon:volume_up
          text: volume_up
          box:
            abs: [183,393 24x24]
            rel: [12,12 24x24]
          style: font:24/400 color:text text:center
    - node: div
      box:
        abs: [20,473 350x48]
        rel: [20,409 350x48]
      layout: flex:row gap:16 justify:center align:center
      flex: shrink:0
      - node: icon-button:chevron_left
        id: review-mode/prev
        mx: ?
        box:
          abs: [76,473 48x48]
          rel: [56,0 48x48]
        layout: flex:row justify:center align:center
        spacing: pad:1/6
        style: r:9999
        - node: icon:chevron_left
          text: chevron_left
          box:
            abs: [88,485 24x24]
            rel: [12,12 24x24]
          style: font:24/400 color:text text:center
      - node: span
        text: Swipe to continue
        box:
          abs: [140,487 111x20]
          rel: [120,14 111x20]
        style: font:13/400/20 color:text-tertiary
      - node: icon-button:chevron_right
        id: review-mode/next
        mx: ?
        box:
          abs: [266,473 48x48]
          rel: [246,0 48x48]
        layout: flex:row justify:center align:center
        spacing: pad:1/6
        style: r:9999
        - node: icon:chevron_right
          text: chevron_right
          box:
            abs: [278,485 24x24]
            rel: [12,12 24x24]
          style: font:24/400 color:text text:center
```

## State: editing (ordered diff vs browsing)

```diff
  - node: span
  text: MEANING
  box:
  abs: [40,148 274x20]
  rel: [0,8 274x20]
  flex: grow:1 basis:0 layout_hint:expanded
  style: font:13/700/20 color:text-tertiary tracking:0.5
- - node: icon-button:edit
- id: review-mode/edit
- mx: ?
- box:
- abs: [314,140 36x36]
- rel: [274,0 36x36]
- layout: flex:row justify:center align:center
- spacing: pad:1/6
- style: r:9999
- - node: icon:edit
- text: edit
- box:
- abs: [322,148 20x20]
- rel: [8,8 20x20]
- style: font:20/400 color:text text:center
- - node: div
- text: school
- box:
- abs: [40,188 310x45]
- rel: [20,68 310x45]
- style: font:30/700/45 color:text
+ - node: icon-button:close
+ id: review-mode/edit
+ mx: ?
+ box:
+ abs: [314,140 36x36]
+ rel: [274,0 36x36]
+ layout: flex:row justify:center align:center
+ spacing: pad:1/6
+ style: r:9999
+ - node: icon:close
+ text: close
+ box:
+ abs: [322,148 20x20]
+ rel: [8,8 20x20]
+ style: font:20/400 color:text text:center
+ - node: div
+ text: school
+ box:
+ abs: [40,188 310x54]
+ rel: [20,68 310x54]
+ spacing: pad:12/16
+ style: font:17/700/26 color:text r:12 border:2px primary
+ - node: span
+ text: |
+ box:
+ abs: [114,203 7x22]
+ rel: [74,15 7x22]
+ style: font:17/700/26 color:primary
+ - node: div
+ box:
+ abs: [40,253 310x38]
+ rel: [20,134 310x38]
+ layout: flex:row gap:8 justify:end
+ - node: btn
+ id: review-mode/edit-cancel
+ mx: ?
+ box:
+ abs: [203,253 76x38]
+ rel: [163,0 76x38]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/16
+ size: minh:38
+ style: r:12
+ - node: span
+ text: Cancel
+ box:
+ abs: [219,264 44x16]
+ rel: [16,11 44x16]
+ style: font:13/700 color:primary-strong text:center
+ - node: btn
+ id: review-mode/edit-save
+ mx: ?
+ box:
+ abs: [287,253 63x38]
+ rel: [247,0 63x38]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/16
+ size: minh:38
+ style: bg:primary r:12 shadow:8/18
+ - node: span
+ text: Save
+ box:
+ abs: [303,264 31x16]
+ rel: [16,11 31x16]
+ style: font:13/700 color:surface text:center
  - node: card
  id: review-mode/term
  box:
  abs: [20,331 350x180]
  rel: [20,267 350x180]
  layout: flex:col gap:12 align:center
  flex: shrink:0
  spacing: pad:24
  position: pos:relative
  style: bg:surface r:20 shadow:9/16
  ...
```

## State: audio (ordered diff vs browsing)

```diff
  - node: div
  text: 학교
  box:
  abs: [152,297 86x72]
  rel: [132,24 86x72]
  style: font:48/800/72 color:text text:center tracking:-1
- - node: icon-button:volume_up
- id: review-mode/audio
- mx: ?
- box:
- abs: [171,381 48x48]
- rel: [151,108 48x48]
- layout: flex:row justify:center align:center
- spacing: pad:1/6
- style: r:9999
- - node: icon:volume_up
- text: volume_up
- box:
- abs: [183,393 24x24]
- rel: [12,12 24x24]
- style: font:24/400 color:text text:center
+ - node: icon-button:graphic_eq
+ id: review-mode/audio
+ mx: ?
+ box:
+ abs: [171,381 48x48]
+ rel: [151,108 48x48]
+ layout: flex:row justify:center align:center
+ spacing: pad:1/6
+ style: r:9999
+ - node: icon:graphic_eq
+ text: graphic_eq
+ box:
+ abs: [183,393 24x24]
+ rel: [12,12 24x24]
+ style: font:24/400 color:text text:center
+ - node: div
+ text: Playing…
+ box:
+ abs: [167,441 56x20]
+ rel: [147,168 56x20]
+ style: font:13/600/20 color:primary text:center
  - node: div
  box:
  abs: [20,504 350x48]
  rel: [20,440 350x48]
  layout: flex:row gap:16 justify:center align:center
  flex: shrink:0
  ...
```

## State: end (ordered diff vs browsing)

```diff
  - node: app__body
  box:
  abs: [0,64 390x716]
  rel: [0,64 390x716]
  layout: flex:col gap:20
  flex: grow:1 basis:0 layout_hint:expanded
  spacing: pad:16/20/96/20
  position: layout_hint:scroll
- - node: div
- box:
- abs: [20,80 350x20]
- rel: [20,16 350x20]
- layout: flex:row gap:12 align:center
- flex: shrink:0
- - node: div
- id: review-mode/progress
- box:
- abs: [20,87 307x6]
- rel: [0,7 307x6]
- position: clip
- style: bg:surface-sunken r:999
- - node: div
- box:
- abs: [20,87 108x6]
- rel: [0,0 108x6]
- style: bg:primary r:999
- - node: span
- text: 7/20
- box:
- abs: [339,80 31x20]
- rel: [319,0 31x20]
- style: font:13/700/20 color:text-secondary
- - node: card
- id: review-mode/meaning
- box:
- abs: [20,120 350x133]
- rel: [20,56 350x133]
- layout: flex:col gap:12
- flex: shrink:0
- spacing: pad:20
- position: pos:relative
- style: bg:surface r:20 shadow:9/16
- - node: div
- box:
- abs: [40,140 310x36]
- rel: [20,20 310x36]
- layout: flex:row align:center
- - node: span
- text: MEANING
- box:
- abs: [40,148 274x20]
- rel: [0,8 274x20]
- flex: grow:1 basis:0 layout_hint:expanded
- style: font:13/700/20 color:text-tertiary tracking:0.5
- - node: icon-button:edit
- id: review-mode/edit
- mx: ?
- box:
- abs: [314,140 36x36]
- rel: [274,0 36x36]
- layout: flex:row justify:center align:center
- spacing: pad:1/6
- style: r:9999
- - node: icon:edit
- text: edit
- box:
- abs: [322,148 20x20]
- rel: [8,8 20x20]
- style: font:20/400 color:text text:center
- - node: div
- text: school
- box:
- abs: [40,188 310x45]
- rel: [20,68 310x45]
- style: font:30/700/45 color:text
- - node: card
- id: review-mode/term
- box:
- abs: [20,273 350x180]
- rel: [20,209 350x180]
- layout: flex:col gap:12 align:center
- flex: shrink:0
- spacing: pad:24
- position: pos:relative
- style: bg:surface r:20 shadow:9/16
- - node: div
- text: 학교
- box:
- abs: [152,297 86x72]
- rel: [132,24 86x72]
- style: font:48/800/72 color:text text:center tracking:-1
- - node: icon-button:volume_up
- id: review-mode/audio
- mx: ?
- box:
- abs: [171,381 48x48]
- rel: [151,108 48x48]
- layout: flex:row justify:center align:center
- spacing: pad:1/6
- style: r:9999
- - node: icon:volume_up
- text: volume_up
- box:
- abs: [183,393 24x24]
- rel: [12,12 24x24]
- style: font:24/400 color:text text:center
- - node: div
- box:
- abs: [20,473 350x48]
- rel: [20,409 350x48]
- layout: flex:row gap:16 justify:center align:center
- flex: shrink:0
- - node: icon-button:chevron_left
- id: review-mode/prev
- mx: ?
- box:
- abs: [76,473 48x48]
- rel: [56,0 48x48]
- layout: flex:row justify:center align:center
- spacing: pad:1/6
- style: r:9999
- - node: icon:chevron_left
- text: chevron_left
- box:
- abs: [88,485 24x24]
- rel: [12,12 24x24]
- style: font:24/400 color:text text:center
- - node: span
- text: Swipe to continue
- box:
- abs: [140,487 111x20]
- rel: [120,14 111x20]
- style: font:13/400/20 color:text-tertiary
- - node: icon-button:chevron_right
- id: review-mode/next
- mx: ?
- box:
- abs: [266,473 48x48]
- rel: [246,0 48x48]
- layout: flex:row justify:center align:center
- spacing: pad:1/6
- style: r:9999
- - node: icon:chevron_right
- text: chevron_right
- box:
- abs: [278,485 24x24]
- rel: [12,12 24x24]
- style: font:24/400 color:text text:center
+ - node: div
+ id: review-mode/end
+ box:
+ abs: [20,80 350x604]
+ rel: [20,16 350x604]
+ layout: flex:col gap:16 justify:center align:center
+ flex: grow:1 basis:0 layout_hint:expanded
+ spacing: pad:40/16
+ - node: icon-tile
+ box:
+ abs: [165,241 60x60]
+ rel: [145,161 60x60]
+ layout: flex:row justify:center align:center
+ flex: shrink:0
+ style: bg:success-soft r:18
+ - node: icon:done_all
+ text: done_all
+ box:
+ abs: [179,255 32x32]
+ rel: [14,14 32x32]
+ style: font:32/400 color:on-success-soft text:center
+ - node: div
+ box:
+ abs: [85,317 220x83]
+ rel: [65,237 220x83]
+ layout: flex:col gap:8
+ size: maxw:220
+ - node: div
+ text: All reviewed
+ box:
+ abs: [85,317 220x30]
+ rel: [0,0 220x30]
+ style: font:20/800/30 color:text text:center tracking:-0.4
+ - node: div
+ text: You've gone through every card in this deck.
+ box:
+ abs: [85,355 220x45]
+ rel: [0,38 220x45]
+ style: font:15/400/23 color:text-secondary text:center
+ - node: div
+ box:
+ abs: [85,416 220x108]
+ rel: [65,336 220x108]
+ layout: flex:col gap:12
+ - node: btn
+ id: review-mode/study-now
+ mx: ?
+ box:
+ abs: [85,416 220x48]
+ rel: [0,0 220x48]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/24
+ size: minh:48
+ style: bg:primary r:12 shadow:8/18
+ - node: icon:school
+ text: school
+ box:
+ abs: [142,430 20x20]
+ rel: [57,14 20x20]
+ style: font:20/400 color:surface text:center
+ - node: span
+ text: Study now
+ box:
+ abs: [170,430 78x19]
+ rel: [85,15 78x19]
+ style: font:15/700 color:surface text:center
+ - node: btn
+ id: review-mode/back-deck
+ mx: ?
+ box:
+ abs: [85,476 220x48]
+ rel: [0,60 220x48]
+ layout: flex:row gap:8 justify:center align:center
+ spacing: pad:0/24
+ size: minh:48
+ style: r:12
+ - node: icon:arrow_back
+ text: arrow_back
+ box:
+ abs: [133,490 20x20]
+ rel: [48,14 20x20]
+ style: font:20/400 color:primary-strong text:center
+ - node: span
+ text: Back to deck
+ box:
+ abs: [161,490 95x19]
+ rel: [76,15 95x19]
+ style: font:15/700 color:primary-strong text:center
```
