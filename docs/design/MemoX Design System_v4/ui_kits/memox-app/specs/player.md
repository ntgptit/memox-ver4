# player — Player — screen spec

> AUTOGENERATED from tool/ui_kit_shots/registry.mjs — do not hand-edit; run `npm run gen:ui-kit`.
> Source of truth: `../_features/player/Player.jsx`; reference PNGs `../shots/player--<state>--<theme>.png` (390×780).
> Verify: `node tool/ui_kit_shots/shoot.mjs player` (gated by `npm run verify:ui-kit`).

## Objective (contract §1)

Play a deck’s cards as hands-free audio.
...
Exact layout order is owned by the source (`../_features/player/Player.jsx`) and the reference PNGs — read those for pixel
...
`MxButton`, `MxContextualAppBar`, `MxFab`, `MxIconButton`, `MxScaffold`, `MxSegmentedControl`, `window.EmptyState`
