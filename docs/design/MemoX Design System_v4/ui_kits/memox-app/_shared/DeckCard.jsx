...
function DeckCard({ icon, tone = 'accent', ring, title, titleWeight, meta, progress, trailing, selected, node }) {
  ...
  if (selected != null) {
    ...
    visual = <div style={{ flexShrink: 0 }}><window.Ring pct={ring} size={40}><span style={{ fontSize: 'var(--memox-font-size-xs)', fontWeight: 'var(--memox-font-weight-bold)' }}>{ring}</span></window.Ring></div>;
  } else {
    ...
    <MxCard padding="sm" interactive variant={selected ? 'primary-soft' : undefined} node={node}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--memox-space-4)', minWidth: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontWeight: titleWeight || 'var(--memox-font-weight-bold)', fontSize: 'var(--memox-font-size-base)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title}</div>
