...
| 11.2 | Repository + migration tests | Transactional multi-table write + rollback + vN→vN+1 migration tests | 3.2, 4.2, 5.2 | 0.4, 0.5, 0.13, DEP-TEST | Specified | P0 | No (schema) | Rollback on failure proven; migrations forward-only + tested; DB source-of-truth asserted | Depends on 0.4, 0.5, 0.13 and DEP-TEST | Opus | TBD |
...
|---|---|---|---|
| TBD — replace with actual squash-merge hash in the next WBS update. | 2026-07-13 | 11.1 | WBS 11.1 Test harness baseline + quality contract: add `docs/testing/quality-contract.md` (test pyramid + tiers with in-repo examples; `__tests__`/`*.test` folder+naming convention; shared-fixtures home `src/shared/testing/`; `npm test` + coverage/command contract; the **Vertical Slice Quality Ownership** rule — every slice ships its own tier-appropriate tests in its own PR). Add shared fixtures `src/shared/testing/fixtures.ts` (`sequentialIds`/`fixedClock`/`tickingClock`, matching the IdGenerator/Clock ports) + test; document `sqlite-test-db.ts` as the shared repository-test harness. Baseline proven by the 20 merged slices. `npm test` 143/143 + `tsc` + `expo lint` clean. No new unblock (2.6 also needs 11.3/11.4). 11.1 → Implemented; counts 21 Implemented / 4 Specified / 42 Blocked |
...
`Implemented` = 0.1 (folder architecture), 0.2 (Expo Router route tree), 0.3 (state-management ADR), 0.10 (UI-Kit→RN mapping), 0.4 (local DB — DEP-DB approved), 0.8 (test-framework decision — DEP-TEST approved), 0.11 (`data-mx-node`→`testID` mapping rule), 1.1 (tokens → typed TS, `src/design-system/tokens/`), 1.2 (ThemeProvider + light/dark, mounted in root layout), 1.3 (Plus Jakarta Sans loaded via `expo-font`, splash-gated), 0.13 (production test harness — jest-expo + RNTL, `npm test` + CI wired), 0.6 (`Result<T,E>` + `AppError` model + user-facing mapping, unit-tested), 0.7 (repository/use-case + transaction/observable contracts in `src/shared/contracts/`, conformance-tested), 3.1 (Deck/subdeck/language domain — pure-TS entities + use cases + no-orphan/no-cycle invariants, unit-tested), 4.1 (Flashcard domain — Card + CardTranslation entities, duplicate detection, validation, unit-tested), 5.1 (Study/session + SRS domain — session state machine + 5-stage ordering + deterministic SM-2 scheduler + resume, unit-tested). The pure-domain layer (3.1/4.1/5.1) is complete, and the persistence framework (0.5) is in and the **persistence layer is complete** — content, flashcard, and session/SRS slices (3.2, 4.2, 5.2) all landed over real SQLite. `Specified` = 0.9, 0.12, 11.2, 11.9 — governance foundation + the repository/migration quality-harness row (11.2←0.5); the test-harness baseline contract (11.1) is now Implemented. `Blocked` = the remaining 42 rows, gated on the icon font (1.4/DEP-ICON-FONT), the feature quality-harness readiness gate (2.6, via 11.1/11.3/11.4), or another unapproved capability (DEP-GOLDEN, DEP-TTS, DEP-NOTIFICATIONS, DEP-FILE-PICKER, DEP-FILE-SHARING, DEP-AUTH, DEP-CLOUD). Notably 1.4–1.7 (need the icon font DEP-ICON-FONT), 2.1/2.2 (need those primitives), the 2.6 gate (needs 11.3/11.4 which need the primitives + DEP-GOLDEN) and every feature slice 3.3–10.3 (gated on 2.6) are Blocked; 21 + 0 + 4 + 42 + 0 + 0 = 67. The critical path to any feature screen now runs through **DEP-ICON-FONT** (1.4 icon adapter → primitives → shell → 11.3/11.4 → 2.6). Production foundation is being laid decision-by-decision; feature screens remain scaffolds until 2.6.
