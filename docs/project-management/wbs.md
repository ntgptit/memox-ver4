...
|---|---|---|---|
| TBD — replace with actual squash-merge hash in the next WBS update. | 2026-07-13 | 0.8 | WBS 0.8 Test framework decision: **DEP-TEST approved** (register → Approved; `jest-expo` + `@testing-library/react-native`). Decision only — no install/config here (that is 0.13). Unblocks 0.13 → Specified. 0.8 → Implemented; counts 6 Implemented / 9 Specified / 52 Blocked |
