...
| 0.8 | Test framework decision | Evaluate the candidate test framework (`jest-expo` + RNTL, DEP-TEST) + coverage tiers (unit/repo/integration/a11y/visual). **Decision gate that decides DEP-TEST — it does not depend on DEP-TEST.** No package install or test config here | 11.1–11.9 | 0.1 | Blocked | P0 | No | Candidate framework evaluated; **Human approves or rejects the candidate** (owner = Human); DEP-TEST updated in the Dependency Approval Register; no package installed and no test config created in this row; only after approval may the test harness setup (0.13) begin | `package.json` (no test framework); `.github/workflows/ui-kit.yml` (existing gate); DEP-TEST | Human | TBD |
...
| 2.4 | Shell/theme state matrix + tests | State-matrix fixtures + interaction + a11y tests for shell + theme | app-bar, theme | 2.1, 2.2, 2.3, 0.13, 0.11, DEP-TEST | Blocked | P1 | Yes | Every app-bar/theme state tested; testID anchors present; framework required (DEP-TEST) | Depends on 0.8; `.../specs/{app-bar,theme}.md` | Opus + Codex | TBD |
...
| 8.2 | Reminder slice | `reminder`: daily study reminder on/off + time-picker; schedule local notifications | reminder (on, off, time-picker) | 0.5, 1.5, 1.7, DEP-NOTIFICATIONS | Blocked | P1 | No | All 3 states; scheduling gated on DEP-NOTIFICATIONS; choice persists; day-chips + time render per shots | `.../specs/reminder.md`, `.../shots/reminder--*.png`; needs DEP-NOTIFICATIONS | Opus | TBD |
...
| 11.2 | Repository + migration tests | Transactional multi-table write + rollback + vN→vN+1 migration tests | 3.2, 4.2, 5.2 | 0.4, 0.5, 0.13, DEP-TEST | Blocked | P0 | No (schema) | Rollback on failure proven; migrations forward-only + tested; DB source-of-truth asserted | Depends on 0.4/0.5/0.8 | Opus | TBD |
...
| 11.4 | Visual regression harness | Choose + configure the golden/screenshot approach; canonical-shot lookup; naming convention; diff-artifact convention; baseline sample for the shell or one fixture | all | 0.8, 0.13, 2.5, DEP-GOLDEN, DEP-TEST | Blocked | P1 | Yes | Golden solution approved (DEP-GOLDEN); harness runs on the baseline sample; diff artifacts are produced and reviewable; each feature slice adds its own canonical screen states; **does not claim golden coverage of all RN screens while those features do not yet exist** | `.../shots/*.png`; needs DEP-GOLDEN; Vertical Slice Quality Ownership (below) | Design + Opus | TBD |
...
Rows **11.1** (test harness + quality contract), **11.3** (accessibility tooling baseline) and **11.4** (visual regression harness) provide **only** the shared harness and contract — they do not implement or complete any feature's tests, and they do not claim app-wide coverage. Each slice consumes those harnesses; because every slice is already `Blocked` on the local-DB decision (0.4/0.5), the harnesses (which land earlier on the critical path) are available before any slice begins its quality work.
...
| DEP-TEST | Production test framework | `jest-expo` + `@testing-library/react-native` | 0.6, 0.8, 0.13, 1.8, 2.4, 3.1, 3.2, 4.1, 4.2, 5.1, 5.2, 11.1, 11.2, 11.3, 11.4 | Pending | Yes |
...
66 work packages total (0.13 added).
...
`Specified` = 0.1, 0.2, 0.3, 0.9, 0.10, 0.11, 0.12, 1.1, 1.2, 1.3, 11.9 — governance + token/theme/font foundation, startable now. `Blocked` = the remaining 55 rows, gated on the local-DB decision (0.4/0.5), the test-framework decision + harness (0.8/0.13), the icon font (1.4/DEP-ICON-FONT), or another unapproved capability (DEP-TEST, DEP-GOLDEN, DEP-TTS, DEP-NOTIFICATIONS, DEP-FILE-PICKER, DEP-FILE-SHARING, DEP-AUTH, DEP-CLOUD). Notably 0.6/0.7 (need the test harness), 1.5/1.6/1.7 (need the icon font 1.4) and 2.1/2.2 (need those primitives) are Blocked; 0 + 0 + 11 + 55 + 0 + 0 = 66. No row is `Implemented`/`Partial` because production is still a scaffold.
