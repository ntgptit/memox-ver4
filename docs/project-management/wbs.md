...
| 0.4 | Local database decision | Evaluate local DB (candidate `expo-sqlite`) as source of truth; schema outline for decks/subdecks/cards/languages/sessions/attempts/SRS; **decision gate that decides DEP-DB — it does not depend on DEP-DB** | all persistence | 0.1 | Blocked | P0 | No | Candidate database evaluated; **Human chooses or rejects the candidate** (owner = Human); Dependency Approval Register updated with the DEP-DB verdict; only after approval do 0.5 and the persistence rows unblock; no schema/database implementation before approval | `package.json` (no DB dep present); Dependency Approval Register (DEP-DB) | Human | TBD |
...
| 10.2 | Account & sync slice | `account-sync`: sign-in + cloud sync states signed-out/in/syncing/conflict/offline | account-sync (5 states) | 5.5, 7.4, DEP-AUTH, DEP-CLOUD | Blocked | P2 | No | All 5 states; the 5-stage study session works (5.5) and session finalization + SRS update work (7.4) before this starts — account/sync must not begin before a usable learning flow; sign-in gated on DEP-AUTH; conflict resolution defined | `.../specs/account-sync.md`, `.../shots/account-sync--*.png` | Opus + Human | TBD |
| 10.3 | Backup & restore | Local backup export + restore import; cloud backup gated on approved provider | account-sync, settings | 4.2, 5.2, 5.5, 7.4, DEP-FILE-PICKER, DEP-FILE-SHARING, DEP-CLOUD | Blocked | P2 | No | Local backup export; local restore import; restore validation before write; backup/restore round-trips losslessly; must not begin before a usable learning flow (5.5 + 7.4); cloud backup only runs when DEP-CLOUD is approved | `.../specs/account-sync.md`; Dependency Approval Register | Opus + Human | TBD |
...
| 11.6 | Android/iOS smoke | Device smoke of the content, learning and portability flows on both platforms | all | 4.6, 5.5, 7.4, 9.1, 9.2, 10.3 | Blocked | P1 | No | On both Android and iOS: content creation and search; start, resume and finish a study session; SRS finalization; import (9.1); export (9.2); backup and restore (10.3); safe area; keyboard; hardware/system back behaviour. Portability smoke is not complete unless 9.1 and 9.2 are done | Device runs | Opus + Human | TBD |
...
`Specified` = 0.1, 0.2, 0.3, 0.6, 0.7, 0.9, 0.10, 0.11, 0.12, 1.1, 1.2, 1.3, 1.5, 1.6, 1.7, 2.1, 2.2, 3.1, 4.1, 5.1, 11.9 — foundational/UI/domain work startable now. `Blocked` = the remaining 44 rows, gated on the local-DB decision (0.4/0.5) or an unapproved capability (DEP-ICON-FONT, DEP-TTS, DEP-NOTIFICATIONS, DEP-FILE-PICKER, DEP-FILE-SHARING, DEP-AUTH, DEP-CLOUD, DEP-TEST, DEP-GOLDEN). No row is `Implemented`/`Partial` because production is still a scaffold.
