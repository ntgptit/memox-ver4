...
// ── 1d. REGRESSION TESTS (contrast / manifest scopes / bundle retired / legacy) ─
const tests = step('design-parity regression tests', spawnSync(node, [join(ROOT, 'tool/tests/design-parity.test.mjs')], { stdio: 'inherit' }));
...
// ── verdict ─────────────────────────────────────────────────────────────────
const pass = drift && contrast && parity && tests && visual && fresh && shotsOk;
